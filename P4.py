...
#Se procede a recrear el proceso realizado en la sección 3.1.3, solamente que cambiando el método de modulación de BPSK al 16-QAM, ESTO ES NECESARIO PARA SIMULAR EL ESQUEMA DIGITAL DE ESTE MÉTODO DE MODULACIÓN
...
def modulador(bits, fc, mpp):
    ...
    for i in range(0, N, 4):
        ...
        senal_Tx[j*mpp: (j+1)*mpp] =             (-1)**(1+bits[i]) * 3**(1-bits[i+1]) * portadora_I             + (-1)**(bits[i+2]) * 3**(1-bits[i+3]) * portadora_Q
        j += 1
...
def demodulador(senal_Rx, portadora_I, portadora_Q, mpp):
    ...
    for i in range(N):
        ...
        # Producto interno de dos funciones
        producto1 = senal_Rx[i*mpp: (i+1)*mpp] * portadora_I
        producto2 = senal_Rx[i*mpp: (i+1)*mpp] * portadora_Q
...
#Se aplica la transformada de fourier a la señal

senal_f = fft.fft(senal_Tx)

#Se obtienen las muestras de la señal

Nm = len(senal_Tx)

#Se obtiene el número de simbolos

Ns = (Nm) // (2*mpp)

#Periodo de la onda portadora
...
#Espacio de frecuencias a graficar 

Ef = np.linspace(0.0, 1.0/(2*Tm), Nm//2)

#Finalmente se grafica la densidad espectral

plt.plot(Ef, 2/Nm * np.power(np.abs(senal_f[0:Nm//2]),2))
plt.xlim(0,20000)
plt.grid()
plt.title('Densidad espectral de potencia')
plt.ylabel(r'$S_{xx}(\omega)$')
